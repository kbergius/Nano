...
namespace Nano
{
	...
	public abstract class Game
	{
        ...
        // these events are triggered from HandleSDLEvents() function
        // note: you can override them to suit your special needs
        protected virtual void OnPointerMove(SDL_MouseMotionEvent ev)
        {

        }
        protected virtual void OnPointerUpDown(SDL_MouseButtonEvent ev)
        {

        }

        protected virtual void OnPointerWheel(SDL_MouseWheelEvent ev)
        {

        }
        protected virtual void OnKeyUpDown(SDL_KeyboardEvent ev)
		{
		
		}

        protected virtual void OnKeyChar(char c)
        {

        }

        protected virtual void OnGamepadAddRemove(SDL_GamepadDeviceEvent ev, bool added)
        {
            ...
            while (SDL.SDL_PollEvent(out var _event))
			{
				switch (_event.type)
				{
					...
					case (uint) SDL.SDL_EventType.SDL_EVENT_GAMEPAD_ADDED:
                        var index = _event.gdevice.which;
                        if (SDL.SDL_IsGamepad(index))
						{
                            OnGamepadAddRemove(_event.gdevice, true);
                        }
                        break;

					case (uint) SDL.SDL_EventType.SDL_EVENT_GAMEPAD_REMOVED:
                        OnGamepadAddRemove(_event.gdevice, false);
                        break;
